...
describe('CalendarPage Component — تست‌های ریدیزاین صفحه انتخاب تقویم رزرو ناهار', () => {
  ...
  describe('۱. سربرگ مینیمال (HEADER)', () => {
    it('باید شامل عنوان بزرگ "رزرو ناهار مدرسه" و زیرعنوان فارسی باشد', () => {
      ...
      expect(subtitleEl?.textContent).toContain('روزهایی که فرزندت در مدرسه غذا دریافت میکند را انتخاب کن');
    });

    it('باید دکمه کوچک "بازگشت" داشته باشد و با کلیک، متد goToHome استور را صدا بزند', () => {
      const compiled = fixture.nativeElement as HTMLElement;
      const backBtn = compiled.querySelector<HTMLButtonElement>('#btn-calendar-back');
      expect(backBtn).toBeTruthy();
      expect(backBtn?.textContent).toContain('بازگشت');
    ...
    it('نباید بج‌های متعدد یا المان‌های اضافه در هدر وجود داشته باشد', () => {
      ...
      expect(header?.textContent).not.toContain('مرحله ۱ از ۲');
    });
  });
